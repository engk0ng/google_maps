//! Geolocation API error types and error messages.

// -----------------------------------------------------------------------------

use crate::geolocation::response::status::Status;
use miette::Diagnostic;
use thiserror::Error;

// -----------------------------------------------------------------------------
//
/// Errors that may be produced by the Google Maps Geolocation API client.

#[derive(Debug, Diagnostic, Error)]
#[diagnostic(code(google_maps::Geolocation::error), url(docsrs))]
pub enum Error {
    /// Forward Geolocation requests (address to latlng) must specify an `address`
    /// or at least one `component`.
    AddressOrComponentsRequired,
    /// Google Maps Geolocation API server generated an error. See the `Status`
    ...
    /// Google Maps Geolocation API server.
    QueryNotBuilt,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::AddressOrComponentsRequired => write!(f,
                ...
                Forward Geolocation requests must specify an `address` or at least one `component`. \
            ...
            Self::GoogleMapsService(status, error_message) => match error_message {
                // If the Google Maps Geolocation API server generated an error
                // message, return that:
                Some(error_message) => write!(f, "Google Maps Geolocation API server: {error_message}"),
                // If the Google Maps Geolocation API server did not generate an
                // error message, return a generic message derived from the
                // response status:
                None => match status {
                    Status::InvalidRequest => write!(f, "Google Maps Geolocation API server: \
                        Invalid request. \
                    ...
                    Status::Ok => write!(f, "Google Maps Geolocation server: \
                        Ok. \
                        The request was successful."),
                    Status::OverDailyLimit => write!(f, "Google Maps Geolocation API server: \
                        Over daily limit. \
                        Usage cap has been exceeded, API key is invalid, billing has not been enabled, or method of payment is no longer valid."),
                    Status::OverQueryLimit => write!(f, "Google Maps Geolocation API server: \
                        Over query limit. \
                        Requestor has exceeded quota."),
                    Status::RequestDenied => write!(f, "Google Maps Geolocation API server: \
                        Request denied \
                        Service did not complete the request."),
                    Status::UnknownError => write!(f, "Google Maps Geolocation API server: \
                        Unknown error."),
                    Status::ZeroResults => write!(f, "Google Maps Geolocation API server: \
                        Zero results. \
            ...
            #[cfg(feature = "enable-reqwest")]
            Self::Reqwest(error) => write!(f, "Google Maps Geolocation API client in the Reqwest library: {error}"),
            #[cfg(feature = "enable-reqwest")]
            Self::ReqwestMessage(error) => write!(f, "Google Maps Geolocation API client in the Reqwest library: {error}"),
            Self::SerdeJson(error) => write!(f, "Google Maps Geolocation API client in the Serde JSON library: {error}"),
        } // match
    } // fn
} // impl
...
impl From<reqwest::Error> for Error {
    ...
    /// Google Maps Geolocation API error type
    /// (`google_maps::Geolocation::error::Error`) by wrapping it inside. This
...
impl From<serde_json::error::Error> for Error {
    ...
    /// error type into a Google Maps Geolocation API error type
    /// (`google_maps::Geolocation::error::Error`) by wrapping it inside. This
    /// function is required to use the `?` operator.
